...
const syncErrorMessageSent = new Map<string, number>()
...
function canSendSyncErrorMessage(from: string): boolean {
...
export async function createOrLoadSession(sessionId: string): Promise<void> {
  ...
  const wasRecentlyCleared = (
    (meta.lastDisconnectCode === 401 || meta.lastDisconnectCode === 515) && 
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
      ...
      for(const m of messages){
        try {
          ...
          try { console.warn('[wa][messages.upsert][err]', sessionId, err && err.message) } catch {}
        }
      }
    })
