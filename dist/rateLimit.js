...
exports.canCreateSession = canCreateSession;
exports.takeSendToken = takeSendToken;
exports.snapshotRateState = snapshotRateState;
const creationWindowMs = 60000;
...
function canCreateSession(ip) {
    ...
    for (const k of Object.keys(creationHits)) {
        ...
        if (!creationHits[k].length)
            delete creationHits[k];
    }
    globalHits = globalHits.filter(ts => now - ts < creationWindowMs);
    if (!creationHits[ip])
        creationHits[ip] = [];
    if (creationHits[ip].length >= creationLimit)
        return { ok: false, reason: 'per_ip_limit' };
    if (globalHits.length >= globalCreationLimit)
...
function takeSendToken(sessionId) {
    const now = Date.now();
    let b = sendBuckets.get(sessionId);
    if (!b) {
        b = { tokens: SEND_BUCKET_CAP, lastRefill: now };
        sendBuckets.set(sessionId, b);
    }
    // refill
...
function snapshotRateState() {
    const send = {};
    for (const [id, b] of sendBuckets.entries())
        send[id] = Number(b.tokens.toFixed(2));
